...
export async function getStoredKey(orgId: string): Promise<StoredKeyData | null> {
...
export async function saveKey(orgId: string, data: StoredKeyData): Promise<void> {
	const dir = getKeysDir();
	await mkdir(dir, { recursive: true });
	await writeFile(getKeyFile(orgId), JSON.stringify(data, null, 2), { mode: 0o600 });
}

export async function updateWrappedOrgKey(orgId: string, wrappedOrgKey: string): Promise<void> {

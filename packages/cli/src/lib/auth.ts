...
function startCallbackServer(): Promise<{ port: number; waitForToken: () => Promise<AuthResult> }> {
...
export function getDefaultUrls(): { siteUrl: string; convexUrl: string; convexSiteUrl: string } {
	return {
...
export function validateUrls(urls: { convexUrl: string; convexSiteUrl: string }): void {
	if (!urls.convexUrl) {
		throw new CliError(
